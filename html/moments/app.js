...
function shareTpl(share){
  var htmlText = '<a href="#"><div class="item-share"><div class="share-img"><img src='+ share.pic +' alt=""></div><span class="share-tt">'+ share.text +'</span></div></a>';
  return htmlText;
}
...
function messageTpl(messageData) {
  ...
  htmlText.push('</div>');
  // 点赞评论菜单
  htmlText.push('<div class="reply-btn-menu"><div class="menu-like"><i class="icon-like"></i>'+ userHasLiked +'</div><div class="menu-comment"><i class="icon-comment"></i>评论</div></div>');
  htmlText.push('</div>');
  // 消息回复模块（点赞和评论）
      ...
      $momentsList.on("click",".menu-like",function(){
        ...
        if ($(this).text() === "点赞") {
          ...
          if ($thisReplyZone.children(".reply-like").length === 0) {
              $thisReplyZone.prepend('<div class="reply-like"><i class="icon-like-blue"></i><a href="#" class="reply-who">'+ userName +'</a></div>');
          }else{
        ...
        var $thisReplyZone = $thisComItem.parents(".item-ft").siblings(".reply-zone");
          if ($(this).hasClass("input-btn-actived")) {
            ...
            if ($thisReplyZone.children(".reply-comment").length) {
              $thisReplyZone.children(".reply-comment").append('<div class="comment-item"><a class="reply-who" href="#">'+ userName +'</a>：'+ $("#comments-input-text").val() +'</div>');
            }else{
              $thisReplyZone.append('<div class="reply-comment"><div class="comment-item"><a class="reply-who" href="#">'+ userName +'</a>：'+ $("#comments-input-text").val() +'</div></div>');
            }
